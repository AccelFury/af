...
- New CLI subcommand under `af <namespace>`? (then
  `.claude/skills/af-add-subcommand` applies)
- New evidence row in `ReusableCoreMaturity`? (then
  `.claude/skills/af-add-evidence-row` applies, and it is breaking for
  `af core verify` consumers)
- New backend adapter under `crates/af-backend-*`?
- New manifest field? (touches `crates/af-manifest`,
  `schemas/af-core.schema.json`, `docs/manifest-reference.md`)
- New registry surface? (touches `registries/`,
