...
inspection, backend planning, wrapper generation, board validation, security, CI
generation, and reports to workspace crates.

Core contracts:

- `af-manifest`: typed TOML parser with compatibility normalization for flat and
  expanded `af-core.toml` shapes.
...
- `af-security`: path normalization, no-shell command execution, command policy
  fields, toolchain manifest parsing, and redaction helpers.
