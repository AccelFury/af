...
Open-source CI в `af` намеренно не запускает закрытые vendor-инструменты без явного opt-in:
...
4. Нет загрузки секретов/секретных файлов (`.env`, `*.pem`, `secret`, `password` и т.п.).
5. Все artifact upload paths лежат в allowlist.
6. Без существующего P&R constraints не создаются board-artifacts.
7. Workflow содержит `pull_request`, `workflow_dispatch` и `permissions: contents: read`.
...
- `af ci run-local` без `--dry-run` проверяет только базовые профили (`sim`, `synth`, `doctor`);
  для `iverilog_make` симуляции он требует `make`, `iverilog` и `vvp`, а для `synth` требует `yosys`.
- Информация о доступности tools/OSS CAD Suite фиксируется в отчётах и может использоваться при выборе инфраструктуры.
